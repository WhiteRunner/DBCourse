<template>
	...
	<div class="c-item" :class="{br: br}" v-else-if="type == 'img'">
		...
		<el-link type="primary" @click="sa.uploadImage(src => {$emit('input', src); sa.ok2('Upload Success');})">Upload</el-link>
	</div>
	<!-- audio -->
	<div class="c-item" :class="{br: br}" v-else-if="type == 'audio'">
		...
		<el-link type="primary" @click="sa.uploadAudio(src => {$emit('input', src); sa.ok2('Upload Success');})">Upload</el-link>
	</div>
	<!-- video -->
	<div class="c-item" :class="{br: br}" v-else-if="type == 'video'">
		<label class="c-label">{{name}}：</label> 
		<el-link type="info" :href="value" target="_blank" v-if="!sa.isNull(value)">{{value}}</el-link>
		<el-link type="primary" @click="sa.uploadVideo(src => {$emit('input', src); sa.ok2('Upload Success');})">Upload</el-link>
	</div>
	<!-- file -->
	<div class="c-item" :class="{br: br}" v-else-if="type == 'file'">
		<label class="c-label">{{name}}：</label> 
		<el-link type="info" :href="value" target="_blank" v-if="!sa.isNull(value)">{{value}}</el-link>
		<el-link type="primary" @click="sa.uploadFile(src => {$emit('input', src); sa.ok2('Upload Success');})">Upload</el-link>
	</div>
	<!-- img-list -->
		...
		<div class="c-item-mline image-box">
			<div class="image-box-2" v-for="item in value_arr">
				...
				<p>
					...
					<el-link @click="value_arr_delete(item)" style="color: #999;">Delete</el-link>
				</p>
			</div>
			<!-- Upload图集 -->
		...
		<div class="c-item-mline">
			<div v-for="item in value_arr">
				...
				<el-link type="danger" class="del-rr" @click="value_arr_delete(item)">
					...
					<small style="vertical-align: top;">Delete</small>
				</el-link>
			</div>
			<el-link type="primary" @click="sa.uploadAudioList(src => value_arr_push(src))" v-if="type == 'audio-list'">Upload</el-link>
			<el-link type="primary" @click="sa.uploadVideoList(src => value_arr_push(src))" v-if="type == 'video-list'">Upload</el-link>
			<el-link type="primary" @click="sa.uploadFileList(src => value_arr_push(src))" v-if="type == 'file-list'">Upload</el-link>
			<el-link type="primary" @click="sa.uploadImageList(src => value_arr_push(src))" v-if="type == 'img-video-list'">Upload Pic</el-link>
			<el-link type="primary" @click="sa.uploadVideoList(src => value_arr_push(src))" v-if="type == 'img-video-list'" style="margin-left: 7px;">Upload Video</el-link>
		</div>
	</div>
	<!-- 富文本 richtext f -->
	...
	<div class="fast-btn" v-else-if="type == 'fast-btn'">
		<el-button type="primary" icon="el-icon-plus" @click="$parent.add()" v-if="showBtns.indexOf('add') != -1">Insert</el-button>
		<el-button type="success" icon="el-icon-view" @click="$parent.getBySelect()" v-if="showBtns.indexOf('get') != -1">Check</el-button>
		<el-button type="danger" icon="el-icon-delete" @click="$parent.deleteByIds()" v-if="showBtns.indexOf('delete') != -1">Delete</el-button>
		<el-button type="warning" icon="el-icon-download" @click="sa.exportExcel()" v-if="showBtns.indexOf('export') != -1">Output</el-button>
		<el-button type="info"  icon="el-icon-refresh"  @click="sa.f5()" v-if="showBtns.indexOf('reset') != -1">Reset</el-button>
		<slot></slot>
	</div>
	<!-- 分页组件 -->
