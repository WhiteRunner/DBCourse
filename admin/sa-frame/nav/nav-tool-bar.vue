...
<template>
	<div class="tools-panel">
		<div class="tools-left">
			<span title="Fold Menu" class="tool-fox" v-if="$root.isOpen == true" @click="$root.endOpen()">
				<i class="el-icon-s-fold"></i>
			</span>
			<span title="Unfold Menu" class="tool-fox" v-if="$root.isOpen == false" @click="$root.startOpen()">
				<i class="el-icon-s-unfold"></i>
			</span>
			<span title="Search-input" class="tool-fox search-fox" :class=" isSearch ? 'search-fox-show' : '' ">
				<el-select v-model="searchText" size="mini" filterable placeholder="Please enter menu keyword" ref="search" 
			...
			<span title="Search Menu" class="tool-fox" @click="closeSearch()" v-if="!isShowSearchInput">
				<i class="el-icon-search" style="font-weight: bold;"></i>
			</span>
			<span title="Search Menu" class="tool-fox" @click="startSearch()" v-else>
				<i class="el-icon-search" style="font-weight: bold;"></i>
			</span>
			<span title="Refresh" class="tool-fox" @click="$root.f5Tab($root.nativeTab)">
		...
		<div class="tools-right">
			<span title="Click To Login" class="tool-fox" onclick="location.href='login.html'" v-if="$root.user == null">
				<span style="font-size: 0.8em; font-weight: bold; position: relative; top: -2px;">Not log in</span>
			</span>
			<span title="My info" class="tool-fox user-info" style="padding: 0;" v-if="$root.user != null">
		...
		<div class="shade-fox" v-if="$root.isDrag" 
			...
			<span style="font-size: 16px;">Open in new window</span>
		</div>
	</div>
</template>
		...
		methods: {
			...
			initInterval: function() {
				...
				this.currInterval = setInterval(function() {
					...
					var z = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][da.getDay()] ; //周几
					// z = z == 0 ? '日' : z;
					var zong = "";
