...
(function() {
    ...
    var cfg_dev = {
            ...
            // api_url: 'http://110.40.199.128:8099', // 所有ajax请求接口父地址
            web_url: 'http://www.baidu.com' // 此项目前台地址 (此配置项非必须)
        }
        // 服务器测试环境
    var cfg_test = {
            api_url: 'http://110.40.199.128:8099',
    ...
    sa.ajax = function(url, data, success200, cfg) {
        ...
        var defaultCfg = {
            ...
            success500: function(res) { // code=500, 代表失败 
                return layer.alert('fail：' + res.msg);
            },
            success403: function(res) { // code=403, 代表权限不足
                return layer.alert("permission denied," + res.msg, { icon: 5 });
            },
            success401: function(res) { // code=401, 代表未登录
                return layer.confirm("Do you want to log in now？", {}, function() {
                    layer.closeAll();
            ...
            errorfn: function(xhr, type, errorThrown) { // ajax发生异常时的默认处理函数
                if (xhr.status == 0) {
                    return layer.alert('Unable to connect to server. Please check network');
                }
                return layer.alert("abnormal：" + JSON.stringify(xhr));
            },
